...
/*                                                        */
/* May 2022                                               */
/* 8.1.1 jolly-dev modified the bootloader in order to make  */
/* it works with the newer ESPTOOL versions - current     */
/* supported version 4.1 (4.2-dev).                       */
/* Added changes to the firmware to allow                 */
/* programming of the ESP8285 using the serial interface  */
/* also used for programming the ATmega328PB              */
/* OPTIBOOT_CUSTOMVER = 1                                 */
/*                                                        */
/* Aug 2019						  						  */
...
* sometimes word pointers. sometimes 16-bit quantities, and sometimes built
* up from 8-bit input characters. Avr-gcc is not great at optimizing the
...
* some code space on parts with smaller page-size by using a smaller int.
*/
#if SPM_PAGESIZE > 255
...
/*
* ESP8285 control signals pins
* The ESP8285 control signals are mapped on the E port of the ATmega328PB
*/
#define EspEnLow()			{ ESP_DDR |= (1<<ESP_ONOFF);			\
							  ESP_PORT &= ~(1<<ESP_ONOFF);	}
#define EspEnHigh()			{ ESP_DDR &= ~(1<<ESP_ONOFF);}
#define EspBootLow()		{ ESP_DDR |= (1<<ESP_BOOT_PIN);			\
							  ESP_PORT &= ~(1<<ESP_BOOT_PIN); }
#define EspBootHigh()		  ESP_DDR &= ~(1<<ESP_BOOT_PIN);
#define EspCheckBootStat()	( (ESP_PIN & (1<<ESP_BOOT_PIN)) >> ESP_BOOT_PIN)
#define EspCheckEnStat()	( (ESP_PIN & (1<<ESP_ONOFF)) >> ESP_ONOFF)

/*
* ESP8285 sync delay routines
* These delay routines are used to sync the bootloader with the ESPTOOL
* in order to correctly put the ESP8285 in programming mode and perform the
* programming task without the need to keep the boot button pressed during
* the whole process
*/
void delay5ms(void)
{
...
#define NUMBER_OF_RX_EDGES		1000

bool firstEdge = false;
uint16_t countOut = NUMBER_OF_RX_EDGES;

/*
* ESP8285 programming mode
*
...
{
	// wait some time before checking the status of the boot pin
	...
	if(EspCheckBootStat() == 0 && EspCheckEnStat() != 0) {
		...
		// tie the ESP's GPIO0 to ground to trigger the boot mode
		EspBootLow();
		delay100ms();
		delay5ms();

		// switch the ESP back on, now we are in boot mode
		EspEnHigh();

		// configure the UART TX pin in INPUT	
		DDRD &= ~(1<<PORTD1);
		
		// wait for the first rising edge on UART RX pin
		while(PIND & (1 << PIND0));
		// communication with ESPtool established
		firstEdge = true;

		// cycle until the end of the programming task
		while(1) {
			// catch the rising edge of the UART RX pin
			if(PIND & (1 << PIND0)) {
				// if this we already caught the first rising edge, then
				// decrease the edges count out.
				if(firstEdge == true){
					if(countOut > 0){
						countOut--;
					}
					// if the count reaches 0, let's wait some more time to
					// be sure there is no remaining byte in the serial interface
					else {
						countOut = NUMBER_OF_RX_EDGES;
						while(countOut-- >0)
							delay5ms();
						
						// release the GPIO0 to exit the boot mode
						EspBootHigh();
						delay300us();
						...
						// configure the UART TX pis to 1 in order to contrast 
						// the ESP's boot message (this should prevent the 
						// propagation of the ESP's boot message outside the
						// Jolly board.   
						DDRD |= (1<<PORTD1);
						PORTD |= (1<<PORTD1);
						...
						// wait some more time to be sure there is no remaining
						// byte in the serial interface
						countOut = NUMBER_OF_RX_EDGES;
						while(countOut-- >0)
							delay300us();
						
						// enable the watchdog to trigger a reboot
						watchdogConfig(WATCHDOG_250MS);
					}
				}
				delay5ms();
			}
			// if we have a falling edge on the UART RX pin, maybe we have 
			// not finished the programming task
			else {
				// reset the "number of edges" used to estimate the communication
				// has come to an end, and wait a bit
				countOut = NUMBER_OF_RX_EDGES;
				delay5ms();
			}
		}
...
int main(void) 
{
	// check if there is an ESP programming event
	espProgModeInvoke();
	/*
